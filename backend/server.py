...
quota_cache = {
    # Gemini models
    "gemini_weekly_percentage": 100.0,
    "gemini_weekly_remaining": "Quota Available",
    "gemini_weekly_refresh": "--",
    "gemini_five_hour_percentage": 100.0,
    "gemini_five_hour_remaining": "Quota Available",
    "gemini_five_hour_refresh": "--",
    
    # Claude/GPT models
    "claude_weekly_percentage": 100.0,
    "claude_weekly_remaining": "Quota Available",
    "claude_weekly_refresh": "--",
    "claude_five_hour_percentage": 100.0,
    "claude_five_hour_remaining": "Quota Available",
    "claude_five_hour_refresh": "--",
...
def parse_section_robust(sec_text):
    ...
    # 1. Weekly Limit Remaining Parsing
    ...
    if weekly_match:
        try:
            ...
            sec_data["weekly_percentage"] = pct
            sec_data["weekly_remaining"] = f"{round(pct)}% remaining"
        except (ValueError, IndexError):
            pass
        
        if len(weekly_match.groups()) >= 2:
            details = weekly_match.group(2).strip()
            if "refreshes in" in details.lower():
                sec_data["weekly_refresh"] = re.sub(r'refreshes in\s*', '', details, flags=re.IGNORECASE).strip()
            elif "remaining" in details.lower():
                sec_data["weekly_remaining"] = details
    ...
    # 2. Five Hour Limit Remaining Parsing
    ...
    if five_hour_match:
        try:
            pct = float(five_hour_match.group(1))
            sec_data["five_hour_percentage"] = pct
            sec_data["five_hour_remaining"] = f"{round(pct)}% remaining"
        except (ValueError, IndexError):
            pass
        ...
        if len(five_hour_match.groups()) >= 2:
            ...
            if "refreshes in" in details.lower():
                sec_data["five_hour_refresh"] = re.sub(r'refreshes in\s*', '', details, flags=re.IGNORECASE).strip()
            elif "remaining" in details.lower():
                sec_data["five_hour_remaining"] = details
    elif "unlimited" in sec_lower or "no limit" in sec_lower:
    ...
    <script>
        ...
        async function fetchMetrics() {
            try {
                ...
                // Gemini Weekly
                const gW_rem = data.gemini_weekly_percentage !== undefined ? data.gemini_weekly_percentage : 100;
                ...
                // Gemini 5-Hour
                const g5_rem = data.gemini_five_hour_percentage !== undefined ? data.gemini_five_hour_percentage : 100;
                ...
                // Claude Weekly
                const cW_rem = data.claude_weekly_percentage !== undefined ? data.claude_weekly_percentage : 100;
                ...
                // Claude 5-Hour
                const c5_rem = data.claude_five_hour_percentage !== undefined ? data.claude_five_hour_percentage : 100;
